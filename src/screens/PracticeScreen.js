...
const D = {
  // ⚠ Light system. The white-alpha values this map used to carry (card at 5%,
  // hair at 10%, faint at 38%) were invisible once the base went white — which is
  // why the grouped cards showed no border and no dividers. Accent hues are
  // darkened too: they were chosen against a dark canvas.
  canvas: COLORS.background, card: COLORS.card,
  ink: COLORS.textPrimary, sub: COLORS.textSecondary, muted: COLORS.textSecondary,
  faint: '#9B9B9B', hair: 'rgba(17,17,17,0.10)',
  indigo: COLORS.primary, indigoSoft: '#E6F6F0',
  blue: '#2563EB', blueSoft: '#E8EEFD',
  cyan: '#0891B2', cyanSoft: '#E0F2FE',
  purple: '#7C3AED', purpleSoft: '#F1EAFE',
  rose: '#BE185D', roseSoft: '#FCE7F0',
  gold: '#B45309', goldSoft: '#FFF4CC',
};
  ...
  if (state.loading) {
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: MCQ_CANVAS }}>
        <StatusBar barStyle="dark-content" backgroundColor={MCQ_CANVAS} />
...
const PracticeScreen = () => {
  ...
  // ── IMPORTANT QUESTIONS LEVEL 3: the chapter screen ─────────────────────────
  ...
  // reader below, which is the same MathJax WebView as before — the chapter view
  ...
  // ── IMPORTANT QUESTIONS LEVEL 3b: the question reader ───────────────────────
  if (impOpen && impSubject && impChapter && impReader) {
    return (
      <QuestionSolveScreen
        subject={{ name: impSubject.name, slug: subjectSlug(impSubject.name) }}
...
const d = StyleSheet.create({
  ...
  typeRow:       { flexDirection: 'row', alignItems: 'center', gap: 14, paddingVertical: 18, paddingHorizontal: 16 },
  ...
  typeTitle:     { fontSize: 16.5, fontFamily: FONT.extrabold, color: D.ink, letterSpacing: -0.3 },
  typeSub:       { fontSize: 13, fontFamily: FONT.semibold, color: D.muted, marginTop: 3 },
