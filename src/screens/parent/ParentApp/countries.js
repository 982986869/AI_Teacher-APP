// src/screens/parent/ParentApp/countries.js
// Country list for the Book-a-trial phone field: name, ISO-2 code, dial code.
// Flag emoji is derived from the ISO-2 code at render time (no image assets needed).

// name, iso2, dial
const RAW = [
  ['Afghanistan', 'AF', '93'], ['Aland Islands', 'AX', '358'], ['Albania', 'AL', '355'],
  ...
  ['Greece', 'GR', '30'], ['Greenland', 'GL', '299'], ['Grenada', 'GD', '1473'],
  ...
  ['Macau', 'MO', '853'], ['Macedonia', 'MK', '389'], ['Madagascar', 'MG', '261'],
  ...
  ['Nigeria', 'NG', '234'], ['North Korea', 'KP', '850'], ['Norway', 'NO', '47'],
  ...
  ['Qatar', 'QA', '974'], ['Reunion', 'RE', '262'], ['Romania', 'RO', '40'],
  ...
  ['Suriname', 'SR', '597'], ['Swaziland', 'SZ', '268'], ['Sweden', 'SE', '46'],
  ...
  ['Uzbekistan', 'UZ', '998'], ['Vanuatu', 'VU', '678'], ['Venezuela', 'VE', '58'],
...
export const COUNTRIES = RAW
  .map(([name, iso2, dial]) => ({ name, iso2, dial: `+${dial}` }))
  .sort((a, b) => a.name.localeCompare(b.name));

// Pinned "popular" countries shown above the divider (matches the reference).
const POPULAR = ['AU', 'CA', 'IN', 'TH', 'AE', 'US'];
export const POPULAR_COUNTRIES = POPULAR
  .map((iso) => COUNTRIES.find((c) => c.iso2 === iso))
  .filter(Boolean);

export const findCountry = (iso2) => COUNTRIES.find((c) => c.iso2 === iso2) || null;
