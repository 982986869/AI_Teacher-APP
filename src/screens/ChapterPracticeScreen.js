...
function QRow({ q, index, onOpen, onToggle }) {
  ...
  return (
    ...
    >
      <Pressable
        ...
        accessibilityLabel={`Question ${qLabel(q, index)}${solved ? ', solved' : ''}`}
      >
        <View style={s.qNum}><Text style={s.qNumTxt}>{qLabel(q, index)}</Text></View>
