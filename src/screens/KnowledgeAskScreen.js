...
import {
  ...
  StatusBar, TextInput, Platform, KeyboardAvoidingView, ActivityIndicator, Image,
} from 'react-native';
...
const KnowledgeAskScreen = ({ onBack }) => {
  ...
  const [docs, setDocs]         = useState([]);   // the user's uploaded documents [{ id, title }]
  const [activeDoc, setActiveDoc] = useState(''); // '' = all documents; else ask only from this one
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = (data?.sources || []).filter((s) => s.status === 'READY' || !s.status);
        const uniq = [...new Set(list.map((s) => (s.subject || '').trim()).filter(Boolean))].sort();
        if (alive) {
          setSubjects(uniq);
          setDocs(list.map((s) => ({ id: s.id, title: s.title })));
        }
      } catch (_) {
  ...
  const handleAsk = async (explicit) => {
    ...
    try {
      const data = await askKnowledgeStructured({
        ...
        sourceIds: activeDoc ? [activeDoc] : undefined,
        history: apiHistory,
      });
        ...
        {tab === 'ask' ? (
          <>
            <ScrollView ref={scrollRef} style={{ flex: 1 }} contentContainerStyle={st.body} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}
              ...
              {/* Ask-from picker — scope questions to one document, or all. */}
              {docs.length >= 2 && (
                <>
                  <Text style={[st.lbl, { marginTop: 14 }]}>Ask from</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.chipRow}>
                    <PressableScale style={[st.chip, !activeDoc && st.chipOn]} onPress={() => setActiveDoc('')}
                      accessibilityLabel="All my files" accessibilityState={{ selected: !activeDoc }}>
                      <Text style={[st.chipTxt, !activeDoc && st.chipTxtOn]}>All my files</Text>
                    </PressableScale>
                    {docs.map((d) => (
                      <PressableScale key={d.id} style={[st.chip, activeDoc === d.id && st.chipOn]} onPress={() => setActiveDoc(d.id)}
                        accessibilityLabel={`Ask from ${d.title}`} accessibilityState={{ selected: activeDoc === d.id }}>
                        <Text style={[st.chipTxt, activeDoc === d.id && st.chipTxtOn, { maxWidth: 180 }]} numberOfLines={1}>{d.title}</Text>
                      </PressableScale>
                    ))}
                  </ScrollView>
                </>
...
const st = StyleSheet.create({
  ...
  sourceRow: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: S.canvas, borderWidth: 1, borderColor: S.hair, borderRadius: R.md, padding: 10, marginBottom: 8 },
