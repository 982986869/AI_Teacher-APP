...
export default function QuestionFormScreen({ route, navigation }) {
  ...
  const initial = useMemo(() => {
    const opts = (question?.options || []).map((o) => (o && o.text != null ? String(o.text) : String(o || '')));
    return {
  ...
  const blankOpt = touched && trimmedOpts.some((o) => !o);
  const dupOpt = new Set(trimmedOpts.filter(Boolean)).size !== trimmedOpts.filter(Boolean).length;
  const textError = touched && !form.text.trim() ? 'Question text is required.' : null;
  const valid = form.text.trim() && trimmedOpts.filter(Boolean).length >= 2 && !trimmedOpts.some((o) => !o);
  ...
  const save = async () => {
    ...
    const body = { question: form.text.trim(), options: trimmedOpts, correctIndex: form.correctIndex, explanation: form.explanation.trim() };
    setSaving(true);
    try {
          ...
          {form.options.map((o, i) => {
            ...
            return (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <TextInput style={[inputStyle, { flex: 1 }]} value={o} onChangeText={(v) => setOpt(i, v)} placeholder={`Option ${LETTERS[i]}`} placeholderTextColor={S.faint} />
