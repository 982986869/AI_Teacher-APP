...
export default function ChapterNotesEditor({ route, navigation }) {
    ...
    getAdminChapterNotes(id)
      .then((d) => {
        ...
        setBlocks((d?.blocks || []).map((b) => {
          const text = htmlToText(b.html != null ? b.html : b.content);
          return { title: b.title || '', text, origHtml: b.html != null ? b.html : null, loaded: text };
        }));
      })
      .catch(() => {})
  ...
  const addBlock = () => setBlocks((bs) => [...bs, { title: '', text: '', origHtml: null, loaded: '' }]);
  ...
  const save = useCallback(async () => {
    ...
    const out = blocks.map((b) => {
      ...
      const html = b.origHtml != null && !changed ? b.origHtml : textToHtml(b.text.trim());
      return { title, html };
