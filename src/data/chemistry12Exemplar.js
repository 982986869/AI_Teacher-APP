// chemistry12Exemplar.js
// Class 12 Chemistry — NCERT Exemplar (chapter-end) questions + solutions.
// Loads the per-chapter raw JSON in ./chemistry12Exemplar/*.json and returns, per
// chapter, a FULL self-contained HTML document (MathJax + card styling) ready to
// drop into a WebView. Mirrors physics12Exemplar.js: the data wraps math in
// {tex}…{/tex}, which we convert to the \( … \) delimiters MathJax renders by
// default.
//
// Chapter keys match ResourcesScreen's Class-12 Chemistry chaptersByClass exactly
// (the data `chapter` field is trimmed of stray trailing whitespace).

import ch01 from './chemistry12Exemplar/01 Solutions.json';
import ch02 from './chemistry12Exemplar/02 Electrochemistry.json';
import ch03 from './chemistry12Exemplar/03 Chemical Kinetics.json';
import ch04 from './chemistry12Exemplar/04 The d- and f- Block Elements.json';
import ch05 from './chemistry12Exemplar/05 Coordination Compounds.json';
import ch06 from './chemistry12Exemplar/06 Haloalkanes and Haloarenes.json';
import ch07 from './chemistry12Exemplar/07 Alcohols Phenols and Ethers.json';
import ch08 from './chemistry12Exemplar/08 Aldehydes Ketones and Carboxylic Acids.json';
import ch09 from './chemistry12Exemplar/09 Amines.json';
import ch10 from './chemistry12Exemplar/10 Biomolecules.json';
import ch11 from './chemistry12Exemplar/11 Chemistry in Everyday Life.json';
import ch12 from './chemistry12Exemplar/12 General Principles and Processes of Isolation of Elements.json';
import ch13 from './chemistry12Exemplar/13 Polymers.json';
import ch14 from './chemistry12Exemplar/14 Solid State.json';
import ch15 from './chemistry12Exemplar/15 Some p-Block Elements.json';
import ch16 from './chemistry12Exemplar/16 Surface Chemistry.json';
...
// Same pastel card template as the PYQ set, with an indigo "Exemplar" tag so the
// two sets read as distinct. pqReport posts {type:'report_error'} to the host.
const HEAD = `
<meta charset="utf-8">
...
<style>
  *{ -webkit-tap-highlight-color:transparent; box-sizing:border-box; }
  ...
  .pq-tag{ font-size:11px; font-weight:700; color:#4F46E5; background:#EEF0FF; padding:4px 10px; border-radius:999px; white-space:nowrap; }
...
const renderCard = (q, qNo) => {
  ...
  const top = `<div class="pq-top"><span class="pq-q">${label}</span><span class="pq-tag">NCERT Exemplar</span></div>`;
  ...
  const foot =
    `<div class="pq-foot"><button class="pq-report" data-qid="chem12exemplar-${q.question_id}" ` +
...
// (trimmed of stray whitespace so it matches the UI chapter list).
const CHEMISTRY12_EXEMPLAR = {};
RAW_CHAPTERS.forEach((questions) => {
  if (Array.isArray(questions) && questions.length > 0 && questions[0].chapter) {
    const key = String(questions[0].chapter).trim();
    CHEMISTRY12_EXEMPLAR[key] = chapterToDoc(questions);
  }
});

// Returns a full HTML document for a Class 12 Chemistry chapter's Exemplar
// questions, or null if there's no local data for it.
export function getChemistry12ExemplarHtml(chapter) {
  return CHEMISTRY12_EXEMPLAR[chapter] || null;
}

// Chapter names that currently have local Exemplar content.
export function getChemistry12ExemplarChapters() {
  return Object.keys(CHEMISTRY12_EXEMPLAR);
}

export default CHEMISTRY12_EXEMPLAR;
