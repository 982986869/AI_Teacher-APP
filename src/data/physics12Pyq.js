// physics12Pyq.js
// Class 12 Physics — Previous Year Questions (PYQ).
// Default-exports a { chapterName: cardsHtml } map, the same shape and styling
// as physics12Important.js. Cards are built at runtime from the per-chapter
// JSON in ./physics12Pyq/*.json so the raw question data stays the source of
// truth. Unlike the Important set, each question carries a `years` array which
// is rendered as the card's tag (e.g. "2025, 2023").
//
// Math stays as {tex}...{/tex} (MathJax renders it via PracticeScreen's
// buildPyqDocument). Images keep their original remote src.
//
// Chapter names match ResourcesScreen's Physics chaptersByClass['Class 12'] and
// PracticeScreen's class-12 chapter list exactly. Only the chapters with a JSON
// file below have data; the rest show "Coming soon" in the chapter list.

import ch01 from './physics12Pyq/01 Electric Charges and Fields.json';
import ch02 from './physics12Pyq/02 Electrostatic Potential and Capacitance.json';
import ch03 from './physics12Pyq/03 Current Electricity.json';
import ch04 from './physics12Pyq/04 Moving Charges and Magnetism.json';
import ch05 from './physics12Pyq/05 Magnetism and Matter.json';
import ch06 from './physics12Pyq/06 Electromagnetic Induction.json';
import ch07 from './physics12Pyq/07 Alternating Current.json';
import ch08 from './physics12Pyq/08 Electromagnetic Waves.json';
import ch09 from './physics12Pyq/09 Ray Optics and Optical Instruments.json';
import ch10 from './physics12Pyq/10 Wave Optics.json';
import ch11 from './physics12Pyq/11 Dual Nature of Radiation and Matter.json';
import ch12 from './physics12Pyq/12 Atoms.json';
import ch13 from './physics12Pyq/13 Nuclei.json';
import ch14 from './physics12Pyq/14 Electronic Devices.json';
...
// Same pastel template as physics12Important.js so PYQ and Important look
...
const renderCard = (q, qNo) => {
  ...
  const foot =
    `<div class="pq-foot"><button class="pq-report" data-qid="phy12pyq-${q.question_id}" ` +
...
RAW_CHAPTERS.forEach((questions) => {
  if (Array.isArray(questions) && questions.length > 0 && questions[0].chapter) {
    PHYSICS12_PYQ[questions[0].chapter] = chapterToHtml(questions);
  }
});

// Returns the PYQ HTML for a Class 12 Physics chapter, or null.
export function getPhysics12PyqHtml(chapter) {
  return PHYSICS12_PYQ[chapter] || null;
}

// Chapter names that currently have content (for availability badges).
export function getPhysics12PyqChapters() {
  return Object.keys(PHYSICS12_PYQ);
}

export default PHYSICS12_PYQ;
