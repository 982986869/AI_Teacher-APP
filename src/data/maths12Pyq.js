// maths12Pyq.js
// Class 12 Mathematics — Previous Year Questions (3164 across 13 chapters).
// Default-exports a { chapterName: cardsHtml } map, the same shape and pastel
// template as chemistry12Pyq.js (Class 12 Chemistry). The cards are built at
// runtime from the per-chapter JSON in ./maths12Pyq/*.json so the raw question
// data stays the source of truth.
//
// Like Class 12 Chemistry (and unlike Class 12 Physics, whose PYQ are served
// from the DB/API), Class 12 Mathematics ships these locally — the same
// self-contained approach used for the Class 12 Maths Important Questions and
// Last Year Papers. PracticeScreen renders the HTML via PyqWebView's `html`
// prop (no API call).
//
// Each question carries a `years` array (the exam years it appeared in) which
// becomes the card's badge (newest first). Math stays as {tex}...{/tex} (MathJax
// renders it via PracticeScreen's buildPyqDocument); images keep their remote src.
//
...
import ch01 from './maths12Pyq/ch01.json'; // Relations and Functions
import ch02 from './maths12Pyq/ch02.json'; // Inverse Trigonometric Functions
import ch03 from './maths12Pyq/ch03.json'; // Matrices
import ch04 from './maths12Pyq/ch04.json'; // Determinants
import ch05 from './maths12Pyq/ch05.json'; // Continuity and Differentiability
import ch06 from './maths12Pyq/ch06.json'; // Application of Derivatives
import ch07 from './maths12Pyq/ch07.json'; // Integrals
import ch08 from './maths12Pyq/ch08.json'; // Application of Integrals
import ch09 from './maths12Pyq/ch09.json'; // Differential Equations
import ch10 from './maths12Pyq/ch10.json'; // Vector Algebra
import ch11 from './maths12Pyq/ch11.json'; // Three Dimensional Geometry
import ch12 from './maths12Pyq/ch12.json'; // Linear Programming
import ch13 from './maths12Pyq/ch13.json'; // Probability

const RAW_CHAPTERS = [
  ch01, ch02, ch03, ch04, ch05, ch06, ch07, ch08, ch09, ch10, ch11, ch12, ch13,
];

// Same pastel template as chemistry12Pyq.js / physics12Pyq so all three subjects
// look identical. pqReport posts a {type:'report_error'} message to the host app.
...
const renderCard = (q, qNo) => {
  ...
  const foot =
    `<div class="pq-foot"><button class="pq-report" data-qid="maths12pyq-${q.question_id}" ` +
...
RAW_CHAPTERS.forEach((questions) => {
  if (Array.isArray(questions) && questions.length > 0 && questions[0].chapter) {
    MATHS12_PYQ[questions[0].chapter] = chapterToHtml(questions);
  }
});

// Returns the PYQ HTML for a Class 12 Mathematics chapter, or null.
export function getMaths12PyqHtml(chapter) {
  return MATHS12_PYQ[chapter] || null;
}

// Chapter names that currently have content (for availability badges).
export function getMaths12PyqChapters() {
  return Object.keys(MATHS12_PYQ);
}

export default MATHS12_PYQ;
