/**
 * build-answer-key-biology.js
 * -----------------------------
 * Fetches the REAL correct answer for every BIOLOGY question from your own API
 * (the /v1/practice/attempted/ endpoint) and writes offline answer files:
 *
 *   1) answer_key_biology.json        -> { questionId: { correctAnswer, correctOptionId, explanation } }
 *   2) <chapter>.with_answers.json      -> chapter questions, each with correctAnswer added
 *   3) answers_biology.txt            -> readable answer sheet, in question order, per chapter
 *
 ...
 *
 * RUN:
 *   1. Put this file + your 12 biology chapter JSONs + index.json in one folder
 *      (the chemistry_questions folder).
 *   2. Paste AUTH_TOKEN and COOKIE below.
 *   3. Test:  node build-answer-key-biology.js --test
 *   4. Full:  node build-answer-key-biology.js
...
const CONFIG = {
  AUTH_TOKEN: 'PASTE_AUTHORIZATION_HEADER_VALUE_HERE',
  COOKIE: 'PASTE_COOKIE_HEADER_VALUE_HERE',

  DIR: '.',                          // folder with chemistry index.json + chapter JSONs
  INDEX_FILE: 'index.json',
  ENDPOINT: 'https://web.examin8.com/v1/practice/attempted/',

  KEY_FILE: 'answer_key_biology.json',
  TXT_FILE: 'answers_biology.txt',
...
async function main() {
  ...
  if (CONFIG.AUTH_TOKEN.startsWith('PASTE_') || CONFIG.COOKIE.startsWith('PASTE_')) {
    console.error('\n  Set AUTH_TOKEN and COOKIE at the top first.\n');
    process.exit(1);
  }
