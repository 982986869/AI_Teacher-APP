/**
 * build-answer-key-chemistry.js
 * -----------------------------
 * Fetches the REAL correct answer for every CHEMISTRY question from your own API
 * (the /v1/practice/attempted/ endpoint) and writes offline answer files:
 *
 *   1) answer_key_chemistry.json        -> { questionId: { correctAnswer, correctOptionId, explanation } }
 *   2) <chapter>.with_answers.json      -> chapter questions, each with correctAnswer added
 *   3) answers_chemistry.txt            -> readable answer sheet, in question order, per chapter
 *
 ...
 *
 * RUN:
 *   1. Put this file + your 12 chemistry chapter JSONs + index.json in one folder
 *      (the chemistry_questions folder).
 *   2. Paste AUTH_TOKEN and COOKIE below.
 *   3. Test:  node build-answer-key-chemistry.js --test
 *   4. Full:  node build-answer-key-chemistry.js
...
const CONFIG = {
  // This API uses cookie-session + CSRF token auth (no Authorization header).
  ...
  KEY_FILE: 'answer_key_chemistry.json',
  TXT_FILE: 'answers_chemistry.txt',
