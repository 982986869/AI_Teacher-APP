// chemistry12Important.js
// Class 12 Chemistry — Important Questions (500 across 10 chapters).
// Default-exports a { chapterName: cardsHtml } map, the same shape and pastel
// template as physics12Important.js (Class 12 Physics). The cards are built at
// runtime from the per-chapter JSON in ./chemistry12Important/*.json so the raw
// question data stays the source of truth.
//
// Unlike Class 12 Physics (whose Important Questions are served from the DB/API),
// Class 12 Chemistry ships these locally — the same self-contained approach used
// for Class 12 Chemistry Last Year Papers. PracticeScreen renders the HTML via
// PyqWebView's `html` prop (no API call).
//
...
import ch01 from './chemistry12Important/ch01.json'; // Solutions
import ch02 from './chemistry12Important/ch02.json'; // Electrochemistry
import ch03 from './chemistry12Important/ch03.json'; // Chemical Kinetics
import ch04 from './chemistry12Important/ch04.json'; // The d- and f- Block Elements
import ch05 from './chemistry12Important/ch05.json'; // Coordination Compounds
import ch06 from './chemistry12Important/ch06.json'; // Haloalkanes and Haloarenes
import ch07 from './chemistry12Important/ch07.json'; // Alcohols Phenols and Ethers
import ch08 from './chemistry12Important/ch08.json'; // Aldehydes Ketones and Carboxylic Acids
import ch09 from './chemistry12Important/ch09.json'; // Amines
import ch10 from './chemistry12Important/ch10.json'; // Biomolecules

const RAW_CHAPTERS = [
  ch01, ch02, ch03, ch04, ch05, ch06, ch07, ch08, ch09, ch10,
];

// Same pastel template as physics12Important.js so Physics and Chemistry look
...
const renderCard = (q, qNo) => {
  ...
  const foot =
    `<div class="pq-foot"><button class="pq-report" data-qid="chem12-${q.question_id}" ` +
...
const CHEMISTRY12_IMPORTANT = {};
RAW_CHAPTERS.forEach((questions) => {
  if (Array.isArray(questions) && questions.length > 0 && questions[0].chapter) {
    CHEMISTRY12_IMPORTANT[questions[0].chapter] = chapterToHtml(questions);
  }
});

// Returns the important-questions HTML for a Class 12 Chemistry chapter, or null.
export function getChemistry12ImportantHtml(chapter) {
  return CHEMISTRY12_IMPORTANT[chapter] || null;
}

// Chapter names that currently have content (for availability badges).
export function getChemistry12ImportantChapters() {
  return Object.keys(CHEMISTRY12_IMPORTANT);
}

export default CHEMISTRY12_IMPORTANT;
