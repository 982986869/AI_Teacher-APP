// physics12Exemplar.js
// Class 12 Physics — NCERT Exemplar (chapter-end) questions + solutions.
// Loads the per-chapter raw JSON in ./physics12Exemplar/*.json and returns, per
// chapter, a FULL self-contained HTML document (MathJax + card styling) ready to
// drop into a WebView. This mirrors the PYQ flow (physics12Pyq.js) but is
// self-contained: the data wraps math in {tex}…{/tex}, which we convert to the
...
import ch04 from './physics12Exemplar/04 Dual Nature of Radiation and Matter.json';
import ch05 from './physics12Exemplar/05 Electric Charges and Fields.json';
import ch06 from './physics12Exemplar/06 Electromagnetic Induction.json';
import ch07 from './physics12Exemplar/07 Electromagnetic Waves.json';
import ch08 from './physics12Exemplar/08 Electronic Devices.json';
import ch09 from './physics12Exemplar/09 Electrostatic Potential and Capacitance.json';
import ch10 from './physics12Exemplar/10 Magnetism and Matter.json';
import ch11 from './physics12Exemplar/11 Moving Charges and Magnetism.json';
import ch12 from './physics12Exemplar/12 Nuclei.json';
import ch13 from './physics12Exemplar/13 Ray Optics and Optical.json';
import ch14 from './physics12Exemplar/14 Wave Optics.json';
...
const renderCard = (q, qNo) => {
  ...
  const foot =
    `<div class="pq-foot"><button class="pq-report" data-qid="phy12exemplar-${q.question_id}" ` +
...
RAW_CHAPTERS.forEach((questions) => {
  if (Array.isArray(questions) && questions.length > 0 && questions[0].chapter) {
    ...
    PHYSICS12_EXEMPLAR[key] = chapterToDoc(questions);
  }
});

// Returns a full HTML document for a Class 12 Physics chapter's Exemplar
// questions, or null if there's no local data for it.
export function getPhysics12ExemplarHtml(chapter) {
  return PHYSICS12_EXEMPLAR[chapter] || null;
}

// Chapter names that currently have local Exemplar content.
export function getPhysics12ExemplarChapters() {
  return Object.keys(PHYSICS12_EXEMPLAR);
}

export default PHYSICS12_EXEMPLAR;
