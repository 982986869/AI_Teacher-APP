...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 9 "Old - Maths" Online Tests (timed MCQ testpapers) into
// ot_tests / ot_questions. examin8 structure:
...
//
// NOTE: paid testpapers return 403 for this account; they are logged and skipped,
// so only the free test(s) per chapter are seeded.
//
// Answers are checkpointed to src/data/class9OnlineTests/<slug>.json so the
// attempt-POSTs never repeat on re-runs.
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass9MathsOldOnlineTests.js            # FETCH (dry)
//   node scripts/seedClass9MathsOldOnlineTests.js --seed --live                              # SEED from checkpoints
// ───────────────────────────────────────────────────────────────────────────
...
const CLASS_LEVEL = 9
const DELAY = 130
const CACHE = path.join(ROOT, 'src', 'data', 'class9OnlineTests')

// Class 9 OLD subjects with online tests. `res` = examin8 content/category id.
const SUBJECTS = [
...
async function seed(docs) {
  ...
  try {
    ...
    for (const doc of docs) {
      ...
      console.log(`  ✓ ${doc.name}: ${doc.chapters.length} chapters, ${nTests} tests, ${nQ} questions (class_level=9)`)
    }
