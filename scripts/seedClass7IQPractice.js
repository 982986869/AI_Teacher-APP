...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 7 "Important Questions" into the generic Practice-tab model
// (subjects → chapters(class_level=7) → sections(type_key='important_questions')
// → questions), so they surface in the Practice tab exactly like Class 12's.
//
// Fetches raw MCQs from examin8 (question + option[] + weightage_name) and maps to
// the questions schema: is_mcq, options[{idx,html,is_correct}], correct_option,
// solution_html, and `year` = weightage badge (VSA/SA/LA).
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass7IQPractice.js          # DRY
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass7IQPractice.js --live    # seed
// ───────────────────────────────────────────────────────────────────────────
...
// name + explicit slug (Hindi has no ASCII, so it needs a manual slug) + flash-card resource id.
const SUBJECTS = [
  { name: 'Science (Curiosity)',                slug: 'science-curiosity',                 res: '24658' },
  { name: 'Social Science (Exploring Society)', slug: 'social-science-exploring-society',  res: '24671' },
  { name: 'हिंदी (मल्हार)',                      slug: 'hindi-malhar',                      res: '24684' },
  { name: 'English (Poorvi)',                   slug: 'english-poorvi',                    res: '24715' },
  { name: 'Maths (Ganita Prakash)',             slug: 'maths-ganita-prakash',              res: '24649' },
  { name: 'Old - Science',                      slug: 'old-science',                       res: '1525'  },
  { name: 'Reasoning & Mental Ability',         slug: 'reasoning-mental-ability',          res: '22546' },
...
// MUST stay byte-identical to the client slugify (src/screens/PracticeScreen.js) so
// DB slugs match the client's API lookups. Devanagari names (empty ASCII base) fall
// back to a stable hash slug — keeps subjects/chapters unique instead of colliding.
const slugify = (s) => {
...
function mapQ(q, i) {
  ...
  return {
    q_number: `Q${i + 1}`,
    year: trim(q.weightage_name) || null,   // badge (VSA/SA/LA)
...
async function fetchSubject(s) {
  // Use the important-questions chapter list (NOT flash-card) — it's the fuller,
  // correct set for IQ (e.g. Hindi 36 vs 7, English 44 vs 14).
  const list = await api(`${B}/content/category/${s.res}/type/0/content_name/important-questions/`)
  const byChapter = {}
  let pos = 0
  const children = list.children || []
  // Flat subject (e.g. Reasoning) — no chapter breakdown; questions live directly on
  // the subject node. Seed them under a single chapter named after the subject.
  if (!children.length) {
...
async function main() {
  ...
  // ONLY=<substr> re-seeds just the matching subject(s) (e.g. ONLY=hindi).
  ...
  try {
    for (const s of data) {
      ...
      // Clean slate: drop this subject's Class 7 chapters (cascades sections/questions)
      // so a re-run with a different chapter list leaves no stale chapters behind.
      ...
      console.log(`  ✓ ${s.name}: ${ci} chapters, ${items} questions (class_level=7)`)
    }
