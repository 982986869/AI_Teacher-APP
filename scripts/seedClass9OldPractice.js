...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 9 "Old - Maths" MCQ Practice content (Practice tab → MCQ Practice)
// into the DB-backed MCQ model:
//   subjects → chapters(class_level=9) → subtopics → mcq_questions.
//
...
// question (with its answer) is CHECKPOINTED to src/data/class9Practice/<slug>.json.
// Re-runs reuse the checkpoint and never re-POST an already-answered question.
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass9MathsOldPractice.js            # FETCH (+checkpoint), dry
//   node scripts/seedClass9MathsOldPractice.js --seed --live                              # SEED from checkpoints
// ───────────────────────────────────────────────────────────────────────────
...
// Class 9 OLD subjects with a practice bank (examin8 resource ids). `res` =
// /practice/topics-q-attempt-data/:res/. Explicit `slug` because 'Old - हिंदी ए/ब'
// both slugify to "old" (ASCII prefix blocks the Devanagari hash fallback).
const SUBJECTS = [
