...
// → questions), mirroring Class 9 (see seedClass9OldIQPractice.js). Fetches raw
// MCQs from examin8 (question + option[] + weightage_name).
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass6OldIQPractice.js          # DRY
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass6OldIQPractice.js --live    # seed
// ───────────────────────────────────────────────────────────────────────────
...
// Class 6 OLD subjects. name + explicit slug (हिंदी slugifies to "old") +
// examin8 content/category (subject) id. Rows are shared with classes 7/8/9.
const SUBJECTS = [
...
function mapQ(q, i) {
  ...
  // Subjective (long-answer) questions carry their answer in `solution`
  // ([{solution:"<html>"}]); MCQs carry it as the correct option's explanation.
  ...
  if (!solution_html) {
    ...
    solution_html = trim(co && co.explanation) || ''
  }
  return {
    q_number: `Q${i + 1}`,
...
async function main() {
  ...
  try {
    for (const s of data) {
      ...
      // Clear ONLY this subject's class-6 important-questions sections (questions
      // cascade) — NOT the chapters (shared with MCQ Practice at class_level=6).
      await client.query(
