...
const PART = 8
// Class 9 OLD subjects with PYQ. `res` = examin8 content/category id.
...
async function fetchSubjectRows(subj) {
  // Enumerate chapters via important-questions (universal); flash-card is empty for
  // subjects without flash cards (English/Hindi Old subjects).
  const list = await api(`${B}/content/category/${subj.res}/type/0/content_name/important-questions/`)
  const chapters = list.children || []
  console.log(`\n=== ${subj.name} (${CLASS_NAME}) PYQ — ${chapters.length} chapters ===`)
  const rows = []
  let ci = 0
  for (const ch of chapters) {
    ...
    if (!qs.length) { await sleep(DELAY); continue }
    ...
    rows.push({ part: PART, subject: subj.name, className: CLASS_NAME, chapter: normApos(ch.name), sectionKey: 'pyq-0', sectionLabel: 'Previous Year Questions', html, chapterPos: ci, position: 0 })
    ci++
    await sleep(DELAY)
  }
  console.log(`  → ${rows.length} chapter-sections with PYQ`)
  return rows
}

async function main() {
  if (!COOKIE || !CSRF) { console.error('Set EXAMIN8_COOKIE and EXAMIN8_CSRF.'); process.exit(1) }
  const outDir = path.join(ROOT, 'src', 'data', 'class9OldPyq')
  ...
  for (const subj of targets) {
    ...
    fs.writeFileSync(path.join(outDir, (subj.slug || subj.name.replace(/[^a-z0-9]+/gi, '-')) + '.json'), JSON.stringify(rows))
    bySubject.push({ subj, rows })
  }
  const total = bySubject.reduce((n, x) => n + x.rows.length, 0)
  console.log(`\nTOTAL: ${total} chapter-sections (part=${PART}) across ${SUBJECTS.length} subjects`)
  ...
  try {
    for (const { subj, rows } of bySubject) {
      ...
      console.log(`  ✓ ${subj.name}: ${rows.length} PYQ sections (part=${PART})`)
    }
