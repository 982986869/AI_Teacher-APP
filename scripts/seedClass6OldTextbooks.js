...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 6 OLD-subject textbook solutions (NCERT + Exemplar) into
// ncert_solutions (className='Class 6'). Books discovered from examin8's
// download-resources `textbook_data` (no hardcoded UUIDs). Each book → its own
// `part` + a display label in ncert_solutions.part_label. Mirror of
// seedClass9OldTextbooks.js.
//
//   part 3         = Exemplar Solutions
//   parts 2,6,7,9… = NCERT books (in textbook order)  [pool avoids 4=notes,5=IQ,8=PYQ]
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass6OldTextbooks.js          # DRY
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass6OldTextbooks.js --live    # seed
// ───────────────────────────────────────────────────────────────────────────
...
// MCQ questions (Exemplar + some NCERT textbook exercises) carry their choices in
// `options_data` ([{ option, explanation, is_correct }]) — NOT in solution_html.
// Render the options (correct one highlighted) + an Answer block with the correct
// letter and its explanation, so the answer key isn't lost.
function optionsHtml(opts) {
  const items = opts.map((o, k) => {
    const L = (OPT_LETTERS[k] || String(k + 1)).toLowerCase()
    const style = o.is_correct
      ? 'border:1px solid #1a7f37;background:#eaf6ec;'
      : 'border:1px solid #ededed;background:#fff;'
    const tick = o.is_correct ? ' <b style="color:#1a7f37;">✓</b>' : ''
    return `<div style="${style}padding:8px 10px;border-radius:8px;margin:6px 0;"><b>(${L})</b> ${trim(o.option)}${tick}</div>`
  }).join('')
  return `<div class="options">${items}</div>`
}
function mcqAnswerHtml(opts) {
  const ci = opts.findIndex((o) => o.is_correct)
  if (ci < 0) return ''
  const L = (OPT_LETTERS[ci] || String(ci + 1)).toLowerCase()
  const expl = trim(opts[ci].explanation)
  const explBlock = expl && expl !== trim(opts[ci].option) ? `<div style="margin-top:6px;">${expl}</div>` : ''
  return `<div class="answer-section"><div class="solution-block"><div class="label">Answer</div><div><b>(${L})</b> ${trim(opts[ci].option)}</div>${explBlock}</div></div>`
}
function ncertCard(q, i) {
  const qh = trim(q.question_html)
  const opts = Array.isArray(q.options) ? q.options.filter((o) => trim(o.option)) : []
  const sh = trim(q.solution_html)
  let body = `<div class="question-text">${qh}</div>`
  if (opts.length) body += optionsHtml(opts)
  let ans = ''
  if (sh) ans = `<div class="answer-section"><div class="solution-block"><div class="label">Solution</div><div>${sh}</div></div></div>`
  else if (opts.length) ans = mcqAnswerHtml(opts)
  return `<div class="question-card"><div class="question-header"><span class="q-number">Q ${i + 1}</span></div><div class="question-body">${body}</div>${ans}</div>`
}
...
async function fetchNode(nodeUuid) {
  ...
  while (url) {
    ...
    for (const q of (j.results || [])) {
      ...
      const options = Array.isArray(q.options_data)
        ? q.options_data.map((o) => ({ option: o.option || '', explanation: o.explanation || '', is_correct: !!o.is_correct }))
        : []
...
async function main() {
  ...
  const outDir = path.join(ROOT, 'src', 'data', 'class6OldTextbooks')
