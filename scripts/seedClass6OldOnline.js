...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 6 OLD-subject Online Tests (timed MCQ testpapers) into
// ot_tests / ot_questions. Mirror of seedClass9OldOnline.js.
//   /v1/category/:subjectId/            → { categories:[chapters] }
//   /v1/category/:chapterId/            → { testpapers:[free], testpapers_paid:[paid] }
//   /v1/quiz/testpaper/:testpaperId/    → { sections:[{questions:[…]}], … }
// Testpaper options don't carry the correct answer; questionIDs are from the
// practice bank, so we reveal it via POST /practice/attempted/ (category =
// chapterId). Paid testpapers 403 → logged and skipped. Answers checkpointed to
// src/data/class6OnlineTests/<slug>.json so attempt-POSTs never repeat.
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass6OldOnline.js            # FETCH (dry)
//   node scripts/seedClass6OldOnline.js --seed --live                              # SEED from checkpoints
// ───────────────────────────────────────────────────────────────────────────
...
const CLASS_LEVEL = 6
const DELAY = 130
const CACHE = path.join(ROOT, 'src', 'data', 'class6OnlineTests')

// Class 6 OLD subjects with online tests (English (OLD) has none → excluded).
const SUBJECTS = [
...
async function fetchSubject(s, cacheFile) {
  ...
  for (const cc of chapterCats) {
    ...
    // The subscribed Class_6 account can read paid testpapers too. Any residual 403
    // (unsubscribed fallback) on a single testpaper is skipped, not fatal.
    ...
    for (const { t, paid: isPaid } of [...free, ...paid]) {
      try {
        ...
        if (e.fatal && /403/.test(e.message)) { skipped++; continue } // paid w/o access — skip
        process.stdout.write(`      ! test ${t.id} failed: ${e.message}\n`); if (e.fatal) throw e
      }
      await sleep(DELAY)
    }
    if (skipped) process.stdout.write(`      (${skipped} test(s) skipped: 403)\n`)
...
async function seed(docs) {
  ...
  try {
    ...
    for (const doc of docs) {
      ...
      console.log(`  ✓ ${doc.name}: ${doc.chapters.length} chapters, ${nTests} tests, ${nQ} questions (class_level=6)`)
    }
