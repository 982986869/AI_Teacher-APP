...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 7 "MCQ Practice" content (Practice tab → MCQ Practice) into the
...
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass7McqPractice.js            # FETCH (+checkpoint), dry
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… ONLY=science node scripts/seedClass7McqPractice.js  # one subject
//   node scripts/seedClass7McqPractice.js --seed --live                              # SEED from checkpoints
// ───────────────────────────────────────────────────────────────────────────
...
const SUBJECTS = [
  ...
  // Reasoning has NO practice bank (practice endpoint → "Invalid category"). Its
  // Important-Questions MCQs already carry correct answers, so we source MCQ Practice
  // from important-questions instead (iq:true → fetchSubjectFromIQ).
  { name: 'Reasoning & Mental Ability',         slug: 'reasoning-mental-ability',         res: '22546', iq: true },
]
...
async function seed(docs) {
  ...
  try {
    for (const doc of docs) {
      ...
      console.log(`  ✓ ${doc.name}: ${doc.chapters.length} chapters, ${totQ} questions (class_level=7)`)
    }
