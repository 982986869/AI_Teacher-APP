...
// ───────────────────────────────────────────────────────────────────────────
// Import CLASS 11 PHYSICS NCERT Solutions (Part-I) into Supabase, mapped into the
// structured schema (supabase/schema.sql) — the SAME generic questions table the
// Class 11 Chemistry / Class 12 NCERT importers use, so the generic /content API
// + the MathJax card renderer serve them unchanged (twin of importChemistry11Ncert):
//
//   subject(Physics) -> chapter(class_level=11) -> section(ncert1) -> questions
//
//   • ncert1 (questions) ← src/data/physics11Ncert1/*.json  (Part-I, 7 files)
//
// Source is the Examin8 per-chapter JSON (identical format to chemistry11Ncert1):
// question_html, solution_html / alternate_solution, options[] (is_correct /
// explanation), q_no, exercise ("Examples" / "Chapter-end"), chapter. The
// exercise label is kept in the q-number chip ("Examples · Q1"), exactly like
// Chemistry. Math stays {tex}…{/tex}; images keep their remote src.
//
// The Class 11 Physics chapters ALREADY EXIST in the DB (created by the exemplar /
// pyq / important importers). This script ATTACHES an ncert1 section to those
// existing chapters by slug — it does NOT reorder them (position/name untouched
// on conflict). Chapter names match existing slugs verbatim, so no NAME_MAP.
//
// Usage:
//   node scripts/importPhysics11Ncert.js          # DRY RUN (parse + report)
//   node scripts/importPhysics11Ncert.js --live    # actually insert into Supabase
//
...
const SECTION = { type: 'ncert1', position: 5, dir: path.join(DATA, 'physics11Ncert1') }
...
// Map a raw question → a `questions` row (twin of importChemistry11Ncert.js).
function mapQuestion(q, position) {
  ...
  // alternate_solution is included so the handful of questions whose only worked
  // solution lives there still render an answer (Chemistry's set had none).
  const solution_html = firstNonEmpty(
    ...
    q.alternate_solution_html, q.alternate_solution_text,
...
async function main() {
  ...
  console.log('\n=== CLASS 11 PHYSICS — NCERT PART-I PARSE REPORT ===')
  let grand = 0
  ...
  console.log(`\nGRAND TOTAL: ${grand} questions across ${order.length} chapters (Class 11 Physics NCERT Part-I)`)

  if (!LIVE) {
    console.log('\n[DRY RUN] Nothing inserted. Live: node scripts/importPhysics11Ncert.js --live\n')
    return
  }
  ...
  try {
    ...
    for (const name of order) {
      // Attach to the EXISTING chapter by slug; keep its current position/name
      // untouched on conflict (no-op update just to RETURN the id).
    ...
    console.log(`\n✓ Class 11 Physics NCERT Part-I import complete — ${chapters} chapters, ${items} questions (class_level=11).`)
  } finally {
    await client.end()
  }
}
