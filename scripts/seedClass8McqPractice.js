...
// ───────────────────────────────────────────────────────────────────────────
// Seed Class 8 "MCQ Practice" content (Practice tab → MCQ Practice) into the
// DB-backed MCQ model:  subjects → chapters(class_level=8) → subtopics → mcq_questions.
//
...
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass8McqPractice.js            # FETCH (+checkpoint), dry
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… ONLY=science node scripts/seedClass8McqPractice.js  # one subject
//   node scripts/seedClass8McqPractice.js --seed --live                              # SEED from checkpoints
// ───────────────────────────────────────────────────────────────────────────
...
const CLASS_LEVEL = 8
const DELAY = 130
const CACHE = path.join(ROOT, 'src', 'data', 'class8Practice')
...
const SUBJECTS = [
  { name: 'Science (Curiosity)',                slug: 'science-curiosity',                res: '26867' },
  { name: 'Social Science (Exploring Society)', slug: 'social-science-exploring-society', res: '27030' },
  { name: 'हिंदी (मल्हार)',                      slug: 'hindi-malhar',                     res: '26769' },
  { name: 'English (Poorvi)',                   slug: 'english-poorvi',                   res: '26892' },
  { name: 'Maths (Ganita Prakash)',             slug: 'maths-ganita-prakash',             res: '26761' },
...
async function seed(docs) {
  ...
  try {
    for (const doc of docs) {
      ...
      console.log(`  ✓ ${doc.name}: ${doc.chapters.length} chapters, ${totQ} questions (class_level=8)`)
    }
