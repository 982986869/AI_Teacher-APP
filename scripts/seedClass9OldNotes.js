...
// ───────────────────────────────────────────────────────────────────────────
// Fetch "Revision Notes" (examin8 flash cards) for Class 9 OLD subjects and seed
// into ncert_solutions as part=4 (className='Class 9'), rendered by Ncert2Screen.
// Only Old - Science & Old - Social Sc expose flash cards.
//
...
//
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass9OldNotes.js          # DRY
//   EXAMIN8_COOKIE=… EXAMIN8_CSRF=… node scripts/seedClass9OldNotes.js --live    # seed
// ───────────────────────────────────────────────────────────────────────────
...
async function main() {
  ...
  const outDir = path.join(ROOT, 'src', 'data', 'class9OldNotes')
  fs.mkdirSync(outDir, { recursive: true })
  const bySubject = []
  for (const subj of SUBJECTS) {
    const rows = await fetchSubjectRows(subj)
    fs.writeFileSync(path.join(outDir, slug(subj.name) + '.json'), JSON.stringify(rows))
    bySubject.push({ subj, rows })
  }
  const total = bySubject.reduce((n, x) => n + x.rows.length, 0)
  console.log(`\nTOTAL: ${total} note sections (part=${PART}) across ${SUBJECTS.length} subjects`)
